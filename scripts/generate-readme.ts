...
import { fetchGeminiOmniPrompts } from './utils/cms-client.js';
...
async function main() {
  ...
  if (downloadVideosMode === 'true' || downloadVideosMode === 'auto') {
    const { prompts } = await fetchGeminiOmniPrompts('en');
  ...
  for (const lang of SUPPORTED_LANGUAGES) {
    ...
    console.log(`  📥 Fetching Gemini Omni prompts (locale: ${lang.code})...`);
    const { prompts, totalDocs } = await fetchGeminiOmniPrompts(lang.code);
