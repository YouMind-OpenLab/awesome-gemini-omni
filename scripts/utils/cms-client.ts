...
export async function fetchGeminiOmniPrompts(locale: string = 'en'): Promise<FetchResult> {
  ...
  const featuredQuery = stringify({
    where: { model: { equals: 'gemini-omni' }, featured: { equals: true } },
